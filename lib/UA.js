import { RTCPeerConnection } from 'react-native-webrtc'; // required from app dependencies
...
module.exports = class UA extends EventEmitter
{
  ...
  {
    console.log('new() [configuration:%o]', configuration);
  ...
  start()
  {
    console.log('start()', this._status, this._closeTimer );
    ...
    {
      console.log('UA is in READY status, not restarted');
    }
    else
    {
      console.log('ERROR: connection is down, Auto-Recovery system is trying to reconnect');
    }
  ...
  get(parameter)
  {
    switch (parameter)
    {
      ...
      default:
        console.log('get() | cannot get "%s" parameter in runtime', parameter);
  ...
  set(parameter, value)
  {
    switch (parameter)
    {
      ...
      default:
        console.log('set() | cannot set "%s" parameter in runtime', parameter);
  ...
  {
    console.log('destroyTransaction', transaction.id)
  ...
  receiveRequest(request)
  {
    ...
    {
      console.log('Request-URI does not point to us');
      if (request.method !== JsSIP_C.ACK)
      {
        request.reply_sl(404);
        this.emit('endCallEvent');
      }
    ...
    if (!request.to_tag)
    {
      switch (method)
      {
        case JsSIP_C.INVITE:
          ...
          {
            console.log('INVITE received but WebRTC is not supported');
            request.reply(488);
          }
          break;
        case JsSIP_C.BYE:
        ...
        case JsSIP_C.CANCEL:
          ...
          {
            console.log('received CANCEL request for a non existent session');
          }
          break;
        case JsSIP_C.ACK:
          /* Absorb it.
    ...
    else
    {
      ...
      else if (method === JsSIP_C.NOTIFY)
      {
        ...
        {
          console.log('received NOTIFY request for a non existent subscription');
      ...
      {
        console.log('CHECK BYE CALL EVENT UA');
  ...
  _loadConfig(configuration)
  {
    ...
    else
    {
      ...
      this._configuration.contact_uri = new URI('sip', this._configuration.display_name, this._configuration.via_host, null, { transport: 'ws' });
    }
      ...
      if (Object.prototype.hasOwnProperty.call(config.settings, parameter))
      {
        switch (parameter)
        {
          ...
          case 'registrar_server':
            console.log(`- ${parameter}: ${this._configuration[parameter]}`);
            break;
          case 'password':
          case 'ha1':
          case 'authorization_jwt':
            ...
            break;
          default:
            console.log(`- ${parameter}: ${JSON.stringify(this._configuration[parameter])}`);
        }
      }
    }
...
{
  console.log('onTransportDisconnect is calling', data)
