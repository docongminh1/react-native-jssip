...
module.exports = class RTCSession extends EventEmitter
{
  ...
  {
    console.log('init_incoming()', request.body);
  ...
  answer(options = {})
  {
    ...
    Promise.resolve()
      ...
      .then(() =>
      {
        console.log('MediaStream answer check local', mediaStream)
        ...
        else if (mediaConstraints.audio || mediaConstraints.video)
        {
          ...
          return mediaDevices.getUserMedia(mediaConstraints)
            .catch((error) =>
            {
              ...
              console.log('emit "getusermediafailed" [error:%o]', error);
      ...
      .then((stream) =>
      {
        console.log('MediaStream answer check RTCPeerconnection id', stream.id)
        if (this._status === C.STATUS_TERMINATED)
        {
          throw new Error('terminated');
        }
        ...
        if (stream)
        {
          ...
          console.log('answer this._connection checkkkk', this._connection)

          this._getPeerConnection(this._connection)
          this._getLocalStreams(stream)
          console.log("🚀 ~ answer ~ stream:", stream)
          // this._connection.onaddstream = e => this._getRemoteStreams(e.stream)
          this._connection.ontrack = e => {
            console.log('answer check this._connection.ontrack e', e)
            this._getRemoteStreams(e.streams)
          }
          // this._connection.ontrack = e => console.log('check this._connection.ontrack e', e)
      ...
      .then(() =>
      {
        ...
        console.log('check offer RTCSessionDescription', offer)
  ...
  _getPeerConnection(connection)
  {
    console.log('getPeerConnection', connection);
   ...
   * Get Stream Session.
   */
  _getLocalStreams(stream)
  {
    console.log('getLocalStreams', stream);
    this.emit('localStream', {
      stream
    });
  }
  _getRemoteStreams(stream)
  {
    console.log('getRemoteStreams', stream);
  ...
  {
    console.log('sendDTMF() | tones: %s', tones);
    ...
    {
      console.log(`"duration" value is lower than the minimum allowed, setting it to ${RTCSession_DTMF.C.MIN_DURATION} milliseconds`);
      duration = RTCSession_DTMF.C.MIN_DURATION;
    }
    else if (duration > RTCSession_DTMF.C.MAX_DURATION)
    {
      console.log(`"duration" value is greater than the maximum allowed, setting it to ${RTCSession_DTMF.C.MAX_DURATION} milliseconds`);
    ...
    {
      console.log(`"interToneGap" value is lower than the minimum allowed, setting it to ${RTCSession_DTMF.C.MIN_INTER_TONE_GAP} milliseconds`);
  ...
  switchCamera()
  {
    ...
    // console.log('switchCamera streams', streams)
    ...
    try {
      ...
      console.log('switchCamera senders', senders);

      senders.forEach(sender => {
        if (sender.track && sender.track.kind === 'video') {
          sender.track._switchCamera()
        }
      });
    } catch (err) {
  ...
  stopLocalCamera()
  {
    ...
    try {
      console.log('stopLocalCamera()');

      const senders = this._connection.getSenders();
      console.log('stopLocalCamera senders', senders);
      ...
      console.log("🚀 ~ stopLocalCamera ~ err:", err)
    }
  }
  ...
  playLocalCamera()
  {
    ...
    try {
      console.log('playLocalCamera()');

      const senders = this._connection.getSenders();
      console.log('playLocalCamera senders', senders);
      ...
      console.log("🚀 ~ playLocalCamera ~ err:", err)
    }
  }
  ...
  _isReadyToReOffer()
  {
    if (!this._rtcReady)
    {
      console.log('_isReadyToReOffer() | internal WebRTC status not ready');
    ...
    if (!this._dialog)
    {
      console.log('_isReadyToReOffer() | session not established yet');
    ...
    {
      console.log('_isReadyToReOffer() | there is another INVITE/UPDATE transaction in progress');
  ...
  _close()
  {
    ...
    {
      console.log('close() | closing local MediaStream');
  ...
  _setACKTimer()
  {
    this._timers.ackTimer = setTimeout(() =>
    {
      if (this._status === C.STATUS_WAITING_FOR_ACK)
      {
        console.log('no ACK received, terminating the session');
  ...
  {
    console.log('_createRTCConnection pcConfig', pcConfig)
    console.log('_createRTCConnection rtcConstraints', rtcConstraints)
    // this._connection = new RTCPeerConnection(pcConfig, rtcConstraints);

    const updatedPCConfig = {
      ...pcConfig,
      sdpSemantics: 'unified-plan'  // Ràng buộc sử dụng Unified Plan
    };
    
    this._connection = new RTCPeerConnection(updatedPCConfig, rtcConstraints);
    ...
    console.log('emit "peerconnection"');
      ...
      .then(() =>
      {
        if (type === 'offer')
        {
          return connection.createOffer(constraints)
            .catch((error) =>
            {
              console.log('emit "peerconnection:createofferfailed" [error:%o]', error);
        ...
        else
        {
          return connection.createAnswer(constraints)
            .catch((error) =>
            {
              console.log('emit "peerconnection:createanswerfailed" [error:%o]', error);
      ...
      .then((desc) =>
      {
        return connection.setLocalDescription(desc)
          .catch((error) =>
          {
            ...
            console.log('emit "peerconnection:setlocaldescriptionfailed" [error:%o]', error);
  ...
  {
    console.log('_processInDialogSdpOffer()');
    ...
    this._connectionPromiseQueue = this._connectionPromiseQueue
      ...
      .then(() =>
      {
        ...
        return this._createLocalDescription('answer', this._rtcAnswerConstraints)
          ...
          {
            request.reply(500);
            console.log('emit "peerconnection:createtelocaldescriptionfailed" [error:%o]', error);
      ...
      .catch((error) =>
      {
        console.log('_processInDialogSdpOffer() failed [error: %o]', error);
      });
  ...
  {
    console.log('receiveRefer()');

    if (!request.refer_to)
    {
      console.log('no Refer-To header field present in REFER');
      request.reply(400);
    ...
    {
      console.log('Refer-To header field points to a non-SIP URI scheme');
      request.reply(416);
  ...
  _receiveReplaces(request)
  {
    ...
    function reject()
    {
      console.log('Replaced INVITE rejected by the user');
      request.reply(486);
    }
  ...
  _sendInitialRequest(mediaConstraints, rtcOfferConstraints, mediaStream)
  {
    ...
    Promise.resolve()
      ...
      .then((stream) =>
      {
        console.log('MediaStream send initial request check RTCPeerconnection id', stream.id)
        if (this._status === C.STATUS_TERMINATED)
        {
          throw new Error('terminated');
        }
        ...
        if (stream)
        {
          ...
          console.log('_sendInitialRequest this._connection checkkkk', this._connection)

          this._getPeerConnection(this._connection)
          this._getLocalStreams(stream)
          console.log("🚀 ~ _sendInitialRequest ~ stream:", stream)
          // this._connection.onaddstream = e => this._getRemoteStreams(e.stream)
          this._connection.ontrack = e => {
            console.log('_sendInitialRequest check this._connection.ontrack e', e)
            this._getRemoteStreams(e.streams)
          }
          // this._connection.ontrack = e => console.log('check this._connection.ontrack e', e)
      ...
      .then((desc) =>
      {
        ...
        console.log('emit "sending" [request:%o]', this._request);
  ...
  _getDTMFRTPSender()
  {
    ...
    {
      console.log('sendDTMF() | no local audio track to send DTMF with');
  ...
  {
    console.log('receiveInviteResponse()');
  ...
  {
    console.log('acceptAndTerminate()');
  ...
  _mangleOffer(sdp)
  {
    ...
    if (this._localHold && !this._remoteHold)
    {
      console.log('mangleOffer() | me on hold, mangling offer');
      for (const m of sdp.media)
      {
        if (holdMediaTypes.indexOf(m.type) === -1)
        {
    ...
    {
      console.log('mangleOffer() | both on hold, mangling offer');
      for (const m of sdp.media)
      {
        if (holdMediaTypes.indexOf(m.type) === -1)
        {
    ...
    {
      console.log('mangleOffer() | remote on hold, mangling offer');
      for (const m of sdp.media)
      {
        if (holdMediaTypes.indexOf(m.type) === -1)
        {
  ...
  _runSessionTimer()
  {
    ...
    if (this._sessionTimers.refresher)
    {
      this._sessionTimers.timer = setTimeout(() =>
      {
        ...
        console.log('runSessionTimer() | sending session refresh request');
    ...
    else
    {
      this._sessionTimers.timer = setTimeout(() =>
      {
        ...
        console.log('runSessionTimer() | timer expired, terminating the session');
  ...
  _toggleMuteAudio(mute)
  {
    ...
    try {
      console.log('_toggleMuteAudio()');

      const senders = this._connection.getSenders();
      console.log('_toggleMuteAudio senders', senders);
      ...
      console.log("🚀 ~ _toggleMuteAudio ~ err:", err)
    }
  }
  ...
  _toggleMuteVideo(mute)
  {
    ...
    try {
      console.log('_toggleMuteVideo()');

      const senders = this._connection.getSenders();
      console.log('_toggleMuteVideo senders', senders);
      ...
      console.log("🚀 ~ _toggleMuteVideo ~ err:", err)
    }
  }
  ...
  _onhold(originator)
  {
    console.log('session onhold');

    this._setLocalMediaStatus();

    console.log('emit "hold"');
  ...
  {
    console.log('session onunhold');

    this._setLocalMediaStatus();

    console.log('emit "unhold"');
  ...
  _onmute({ audio, video })
  {
    console.log('session onmute');

    this._setLocalMediaStatus();

    console.log('emit "muted"');
  ...
  {
    console.log('session onunmute');

    this._setLocalMediaStatus();

    console.log('emit "unmuted"');
